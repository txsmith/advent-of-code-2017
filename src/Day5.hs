...
answer part = do
  ...
  let
    loop = do
      ...
      case part of
        One -> modify instructions (+1) ptr
        Two -> if instr < 3 
                then modify instructions (+1) ptr
                else modify instructions (subtract 1) ptr
      writeIORef instrPtr (ptr+instr)
      modifyIORef stepCounter (+1)
      pure (ptr+instr)

  iterateUntil isOutOfBounds loop
